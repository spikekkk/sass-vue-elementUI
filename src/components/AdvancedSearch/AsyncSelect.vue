...
export default {
  ...
  // },
  mounted() {
    const { isAsync, initFirst, options, key, optionValue } = this.field
    if (isAsync) {
      this.getSelect()
    } else {
      this.list = options
      if (initFirst && options.length > 0) {
  ...
  methods: {
    async getSelect() {
      const { url, asyncParams, initFirst, key, optionValue, listValue } = this.field
      this.loading = true
