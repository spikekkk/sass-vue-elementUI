...
export default {
  ...
  methods: {
    ...
    linePayFun() {
      queryAuditStatus().then(res => {
        if (res.data.errorCode === 0) {
          this.gatheringData.contractOrderId = this.coontractRowlist.orderNumber
    ...
    updateGatheringPerMoney() {
      this.gatheringData.perMoney = (this.contractOrderDetail.totalMoney - this.contractOrderDetail.receivedMoney - parseFloat(this.depositSelect && this.depositSelect.money || 0)).toFixed(2)
    ...
    payOrderFun() {
      ...
      if (this.gatheringData.payType === '' || this.gatheringData.payType === undefined || this.gatheringData.payType === null) {
        this.$message.error('收款方式不能为空')
      } else if (this.gatheringData.perMoney === '' || this.gatheringData.perMoney === undefined || this.gatheringData.perMoney === null) {
    ...
    cancelHandsBtn() {
      ...
      this.gatheringData.realMoney = this.gatheringData.needPayMoney
    ...
    perMoneyChange(value) {
      // 用数字比较大小
      const perMoneyNum = Number(value)
      const needPayMoneyNum = Number(this.gatheringData.needPayMoney)
      if (perMoneyNum < 0) {
        this.gatheringData.perMoney = 0
      } else if (value === '' || value === undefined || value === null) {
        this.gatheringData.perMoney = needPayMoneyNum
      } else if (perMoneyNum > needPayMoneyNum) {
        this.$nextTick(() => {
          this.gatheringData.perMoney = needPayMoneyNum
        })
      } else if (perMoneyNum >= 0 && perMoneyNum < needPayMoneyNum) {
      ...
      // 判断是否有利率
      if (this.gatheringData.rateInfo === '' || this.gatheringData.rateInfo === undefined || this.gatheringData.rateInfo === null) {
        this.gatheringData.realMoney = this.gatheringData.perMoney
      } else {
        this.gatheringData.realMoney = this.gatheringData.perMoney * (1 - this.gatheringData.rateInfo)
      }
    }
  }
}
