                ...
                <div class="from_con_body">
                  ...
                  <div style="width: 85%">
                    ...
                    <el-select
                      ...
                      placeholder="请选择收款方式"
                      style="width: 42%"
                      @input="payTypeChange(addGatheringData.payType)"
                    >
                      <el-option
...
<script>
import {
  summaryQuery, // 定金管理---新建定金弹框---学员下拉列表
...
export default {
  ...
  methods: {
    ...
    updateGatheringPerMoney() {
      this.addGatheringData.perMoney = (this.contractOrderDetail.totalMoney - this.contractOrderDetail.receivedMoney - parseFloat(this.depositSelect && this.depositSelect.money || 0)).toFixed(2)
    ...
    payOrderFun() {
      ...
      if (this.addGatheringData.payType === '' || this.addGatheringData.payType === undefined || this.addGatheringData.payType === null) {
        this.$message.error('收款方式不能为空')
      } else if (this.addGatheringData.perMoney === '' || this.addGatheringData.perMoney === undefined || this.addGatheringData.perMoney === null) {
    ...
    perMoneyChange(value) {
      ...
      const needPayMoneyNum = Number(this.addGatheringData.needPayMoney)
      if (perMoneyNum < 0) {
        this.addGatheringData.perMoney = 0
      } else if (value === '' || value === undefined || value === null) {
        this.addGatheringData.perMoney = needPayMoneyNum
      ...
      // 判断是否有利率
      if (this.addGatheringData.rateInfo === '' || this.addGatheringData.rateInfo === undefined || this.addGatheringData.rateInfo === null) {
        this.addGatheringData.realMoney = this.addGatheringData.perMoney
      } else {
        this.addGatheringData.realMoney = this.addGatheringData.perMoney * (1 - this.addGatheringData.rateInfo)
      }
    },
