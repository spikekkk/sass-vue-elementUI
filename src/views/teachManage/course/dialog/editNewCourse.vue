...
<script>
import { getEditInfo, queryCoursegroup, updateCourse } from '@/api/teachManage/course'
export default {
  data() {
    /* 整数校验 */
    ...
    return {
      ...
      rules: {
        ...
        cost: [
          { required: true, message: '填写每节课程所需的课时数', trigger: 'change' },
        ...
        if (valid) {
          ...
          updateCourse(params).then(res => {
            ...
            if (data.errorCode === 0) {
              ...
              this.$message.error(data.errorMessage || '编辑课程失败')
            }
          })
        } else {
          return false
